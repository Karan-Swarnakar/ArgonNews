...
import { X, List, Columns, SlidersHorizontal } from 'lucide-react';
...
export const FilterBar: React.FC<FilterBarProps> = ({
  ...
  const viewModes: { id: ViewMode; label: string; icon: React.FC<{ className?: string }> }[] = [
    { id: 'editorial', label: 'Broadsheet Dispatch', icon: Columns },
    { id: 'dense', label: 'Wire Scan', icon: List },
  ];

  return (
    <div className="border-b border-[#22272e] bg-[#0c0e11] py-2 text-xs">
        ...
        <div className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap min-h-[28px]">
            {filters.selectedEntity ? (
              <span className="inline-flex items-center gap-1.5 rounded bg-[#1c2128] border border-[#38414e] px-2.5 py-1 text-[11px] font-sans text-amber-200">
                ...
                <span className="font-medium text-[#f1f5f9]">{filters.selectedEntity}</span>
                <button
                  onClick={() => onFilterChange({ selectedEntity: undefined })}
                  className="hover:text-white ml-1 text-[#8b949e]"
                  title="Clear filter"
                >
                  <X className="h-3 w-3" />
                </button>
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <div className="flex items-center gap-1">
              <span className="font-sans text-[11px] uppercase text-[#6e7681] hidden sm:inline">
                Source:
              </span>
              <select
                ...
                className="rounded border border-[#30363d] bg-[#14161a] px-2.5 py-1 text-xs text-[#c9d1d9] focus:border-[#58a6ff] focus:outline-none font-sans max-w-[200px] truncate"
              >
            ...
            <div className="flex items-center gap-1">
              ...
              <select
                ...
                className="rounded border border-[#30363d] bg-[#14161a] px-2.5 py-1 text-xs text-[#c9d1d9] focus:border-[#58a6ff] focus:outline-none font-sans"
              >
                <option value="newest">Newest First</option>
                <option value="importance-desc">Highest Impact</option>
              </select>
            </div>
            ...
            {isFiltered && (
              <button
                ...
                className="inline-flex items-center gap-1 rounded border border-[#38414e] bg-[#1c2128] px-2 py-1 text-[11px] font-sans text-[#c9d1d9] hover:text-white transition-colors"
